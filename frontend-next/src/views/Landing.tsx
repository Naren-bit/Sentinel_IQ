    ...
    <div className="min-h-screen bg-[var(--background)] relative overflow-hidden font-sans">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-10 py-4 bg-white/70 backdrop-blur-[20px] border-b border-white/40 shadow-[0_4px_20px_rgba(31,41,55,0.02)]">
        ...
        <div className="hidden md:flex items-center gap-8 text-[var(--on-surface-variant)] text-sm font-medium">
          <button onClick={reset} className="text-[var(--primary)] border-b-2 border-[var(--primary)] pb-1 cursor-pointer">Dashboard</button>
          <button onClick={() => setScreen('upload')} className="hover:text-[var(--on-surface)] transition-colors pb-1 cursor-pointer">Review Queue</button>
          <button onClick={() => {}} className="hover:text-[var(--on-surface)] transition-colors pb-1 cursor-pointer">Audit Logs</button>
          <button onClick={() => {}} className="hover:text-[var(--on-surface)] transition-colors pb-1 cursor-pointer">Settings</button>
        </div>
        <Button 
