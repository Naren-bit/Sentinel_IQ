...
export function Landing() {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden font-sans">
      ...
      {/* Top Navbar - Dark Glassmorphism Pill */}
      <nav className="fixed top-6 left-1/2 -translate-x-1/2 w-[95%] max-w-5xl z-50 flex items-center justify-between px-4 py-3 bg-[#1a1a1a]/70 backdrop-blur-3xl border border-white/10 shadow-[0_16px_40px_rgba(0,0,0,0.3)] rounded-full">
        <div className="flex items-center gap-2 pl-4">
          <Shield className="w-6 h-6 text-[#9d4edd] fill-[#9d4edd]" />
          <span className="text-xl font-bold text-white tracking-tight">SentinelIQ</span>
        </div>
        <div className="hidden md:flex items-center gap-2 text-sm font-medium">
          <button onClick={reset} className="bg-[#9d4edd] text-white px-6 py-2 rounded-full cursor-pointer shadow-[0_0_20px_rgba(157,78,221,0.4)]">Dashboard</button>
          <button onClick={() => setScreen('upload')} className="text-white/60 hover:text-white hover:bg-white/5 transition-all px-4 py-2 rounded-full cursor-pointer">Review Queue</button>
          <button onClick={() => {}} className="text-white/60 hover:text-white hover:bg-white/5 transition-all px-4 py-2 rounded-full cursor-pointer">Audit Logs</button>
          <button onClick={() => {}} className="text-white/60 hover:text-white hover:bg-white/5 transition-all px-4 py-2 rounded-full cursor-pointer">Settings</button>
        </div>
        <Button 
